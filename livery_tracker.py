...
url = ''  # Insert your discord webhook url here
        ...
        while True:
            ...
            try:
                while True:
                    ...
                    if r.status_code == 429:  # We are rate limited, see https://discord.com/developers/docs/topics/rate-limits
                        retry_time = time.time() + int(r.headers["retry-after"])
                        logger.debug(
                            f"We are rate limited, text: {r.text}\n Will retry after {int(retry_time - time.time())}")
                        first_append = True
                        while time.time() < retry_time:
                            try:
                                message_from_queue = self.queue.get(block=False)
                                if message_from_queue == "exit":
                                    isStopped = True
                                    break
                                message_from_queue = requests.utils.quote(message_from_queue)
                            except queue.Empty:
                                continue
                            if first_append:
                                message = "\n" + message + "\n" + message_from_queue
                                ...
                                message = message + "\n" + message_from_queue
                                logger.debug("Add new message for send")
                        continue
